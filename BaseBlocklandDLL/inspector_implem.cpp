...
#include "out.h"  // NOLINT(build/include_order)
	};
	...
	void SendVersionResponse(InspectorSocket* socket) {
		...
		response["Browser"] = "blocklandjs/" + std::string(BLJS_VERSION);
...
bool InspectorSocketServer::Start() {
	...
	if (server_sockets_.empty()) {
		if (out_ != nullptr) {
			Printf("Starting inspector on %s:%d failed: %s\n",
